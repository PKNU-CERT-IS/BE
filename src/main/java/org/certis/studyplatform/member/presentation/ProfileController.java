...
public class ProfileController {
     ...
     */
    @GetMapping("/me")
    public ResponseEntity<GlobalResponseHandler<ProfileVo>> getMyProfile(
            @AuthenticationPrincipal CurrentUser currentUser) {
        log.info("REST: Getting my profile for user ID: {}", currentUser.getId());

        ProfileVo profileVo = profileFacadeService.getMyProfile(currentUser.getId());

        log.info("REST: My profile retrieved successfully for user ID: {}", currentUser.getId());

        return GlobalResponseHandler.success(ResponseStatus.PROFILE_FIND_SUCCESS, profileVo);
    }
     ...
     */
    @GetMapping("/study")
    public ResponseEntity<GlobalResponseHandler<List<ProfileStudyVo>>> getMyStudies(
            @AuthenticationPrincipal CurrentUser currentUser) {
        log.info("REST: Getting my studies for user ID: {}", currentUser.getId());

        List<ProfileStudyVo> studies = profileFacadeService.getMyStudies(currentUser.getId());
     ...
     */
    @GetMapping("/project")
    public ResponseEntity<GlobalResponseHandler<List<ProfileProjectVo>>> getMyProjects(
            @AuthenticationPrincipal CurrentUser currentUser) {
        log.info("REST: Getting my projects for user ID: {}", currentUser.getId());

        List<ProfileProjectVo> projects = profileFacadeService.getMyProjects(currentUser.getId());
     ...
     */
    @GetMapping("/blog")
    public ResponseEntity<GlobalResponseHandler<List<ProfileBlogVo>>> getMyBlogs(
            @AuthenticationPrincipal CurrentUser currentUser) {
        log.info("REST: Getting my blogs for user ID: {}", currentUser.getId());

        List<ProfileBlogVo> blogs = profileFacadeService.getMyBlogs(currentUser.getId());
