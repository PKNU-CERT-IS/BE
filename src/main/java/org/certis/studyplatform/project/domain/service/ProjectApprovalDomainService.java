    ...
    private void validateApprovalPermission(Long approverId) {
        try {
            ...
            if (!MemberRole.isStaffOrAbove(role)) {
                throw new DomainException(ExceptionStatus.PROJECT_DOMAIN_ACCESS_DENIED, "승인 권한이 없습니다");
            }
            log.debug("Domain: Approval permission granted - approverId: {}, role: {}", approverId, role);
        } catch (DomainException e) {
            throw e;
        } catch (Exception e) {
            throw new DomainException(ExceptionStatus.PROJECT_DOMAIN_ACCESS_DENIED, "승인 권한 검증 중 오류가 발생했습니다");
        }
    }
